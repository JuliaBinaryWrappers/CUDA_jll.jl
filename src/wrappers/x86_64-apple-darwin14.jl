...
export libcufftw, libcusolver, libcusparse, libcudart, nvdisasm, libnppist, libnppitc, libnppicom, libnppif, libnppicc, libcurand, libnppim, libnppidei, libcupti, libcublas, libcufft, libnppial, libcudadevrt, libnvvm, libnppig, libcurand, libnpps, libnvblas, libdevice, libnppisu, libnppc, libnvtoolsext
...
const libcufftw_splitpath = ["lib", "libcufftw.10.0.dylib"]
...
const libcufftw = "@rpath/libcufftw.10.0.dylib"


# Relative path to `libcusolver`
const libcusolver_splitpath = ["lib", "libcusolver.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusolver = "@rpath/libcusolver.10.0.dylib"


# Relative path to `libcusparse`
const libcusparse_splitpath = ["lib", "libcusparse.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusparse = "@rpath/libcusparse.10.0.dylib"


# Relative path to `libcudart`
const libcudart_splitpath = ["lib", "libcudart.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "@rpath/libcudart.10.0.dylib"
...
const libnppist_splitpath = ["lib", "libnppist.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "@rpath/libnppist.10.0.dylib"


# Relative path to `libnppitc`
const libnppitc_splitpath = ["lib", "libnppitc.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppitc = "@rpath/libnppitc.10.0.dylib"


# Relative path to `libnppicom`
const libnppicom_splitpath = ["lib", "libnppicom.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicom = "@rpath/libnppicom.10.0.dylib"


# Relative path to `libnppif`
const libnppif_splitpath = ["lib", "libnppif.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppif = "@rpath/libnppif.10.0.dylib"


# Relative path to `libnppicc`
const libnppicc_splitpath = ["lib", "libnppicc.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "@rpath/libnppicc.10.0.dylib"


# Relative path to `libcurand`
const libcurand_splitpath = ["lib", "libcurand.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "@rpath/libcurand.10.0.dylib"


# Relative path to `libnppim`
const libnppim_splitpath = ["lib", "libnppim.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppim = "@rpath/libnppim.10.0.dylib"


# Relative path to `libnppidei`
const libnppidei_splitpath = ["lib", "libnppidei.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "@rpath/libnppidei.10.0.dylib"


# Relative path to `libcupti`
const libcupti_splitpath = ["lib", "libcupti.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcupti = "@rpath/libcupti.10.0.dylib"


# Relative path to `libcublas`
const libcublas_splitpath = ["lib", "libcublas.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcublas = "@rpath/libcublas.10.0.dylib"


# Relative path to `libcufft`
const libcufft_splitpath = ["lib", "libcufft.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcufft = "@rpath/libcufft.10.0.dylib"


# Relative path to `libnppial`
const libnppial_splitpath = ["lib", "libnppial.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "@rpath/libnppial.10.0.dylib"
...
const libnppig_splitpath = ["lib", "libnppig.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppig = "@rpath/libnppig.10.0.dylib"


# Relative path to `libcurand`
const libcurand_splitpath = ["lib", "libnvgraph.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "@rpath/libnvgraph.10.0.dylib"


# Relative path to `libnpps`
const libnpps_splitpath = ["lib", "libnpps.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnpps = "@rpath/libnpps.10.0.dylib"


# Relative path to `libnvblas`
const libnvblas_splitpath = ["lib", "libnvblas.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "@rpath/libnvblas.10.0.dylib"
...
const libnppisu_splitpath = ["lib", "libnppisu.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "@rpath/libnppisu.10.0.dylib"


# Relative path to `libnppc`
const libnppc_splitpath = ["lib", "libnppc.10.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppc = "@rpath/libnppc.10.0.dylib"
