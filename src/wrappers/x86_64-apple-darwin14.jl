...
export libnppitc, libnppisu, libnppidei, libnvblas, libnppist, libdevice, libnvtoolsext, libcurand, libcurand, libnpps, libnppicom, libcusparse, libnppicc, libcusolver, libcudadevrt, libnppif, libnppim, libcudart, libnppc, libcufftw, libcufft, libnppial, libnppig, nvdisasm, libcupti, libnvvm, libcublas
...
const libnppitc_splitpath = ["lib", "libnppitc.9.2.dylib"]
...
const libnppitc = "@rpath/libnppitc.9.2.dylib"


# Relative path to `libnppisu`
const libnppisu_splitpath = ["lib", "libnppisu.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "@rpath/libnppisu.9.2.dylib"


# Relative path to `libnppidei`
const libnppidei_splitpath = ["lib", "libnppidei.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "@rpath/libnppidei.9.2.dylib"


# Relative path to `libnvblas`
const libnvblas_splitpath = ["lib", "libnvblas.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "@rpath/libnvblas.9.2.dylib"


# Relative path to `libnppist`
const libnppist_splitpath = ["lib", "libnppist.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "@rpath/libnppist.9.2.dylib"
...
const libcurand_splitpath = ["lib", "libnvgraph.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "@rpath/libnvgraph.9.2.dylib"


# Relative path to `libcurand`
const libcurand_splitpath = ["lib", "libcurand.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "@rpath/libcurand.9.2.dylib"


# Relative path to `libnpps`
const libnpps_splitpath = ["lib", "libnpps.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnpps = "@rpath/libnpps.9.2.dylib"


# Relative path to `libnppicom`
const libnppicom_splitpath = ["lib", "libnppicom.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicom = "@rpath/libnppicom.9.2.dylib"


# Relative path to `libcusparse`
const libcusparse_splitpath = ["lib", "libcusparse.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusparse = "@rpath/libcusparse.9.2.dylib"


# Relative path to `libnppicc`
const libnppicc_splitpath = ["lib", "libnppicc.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "@rpath/libnppicc.9.2.dylib"


# Relative path to `libcusolver`
const libcusolver_splitpath = ["lib", "libcusolver.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusolver = "@rpath/libcusolver.9.2.dylib"
...
const libnppif_splitpath = ["lib", "libnppif.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppif = "@rpath/libnppif.9.2.dylib"


# Relative path to `libnppim`
const libnppim_splitpath = ["lib", "libnppim.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppim = "@rpath/libnppim.9.2.dylib"


# Relative path to `libcudart`
const libcudart_splitpath = ["lib", "libcudart.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "@rpath/libcudart.9.2.dylib"


# Relative path to `libnppc`
const libnppc_splitpath = ["lib", "libnppc.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppc = "@rpath/libnppc.9.2.dylib"


# Relative path to `libcufftw`
const libcufftw_splitpath = ["lib", "libcufftw.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcufftw = "@rpath/libcufftw.9.2.dylib"


# Relative path to `libcufft`
const libcufft_splitpath = ["lib", "libcufft.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcufft = "@rpath/libcufft.9.2.dylib"


# Relative path to `libnppial`
const libnppial_splitpath = ["lib", "libnppial.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "@rpath/libnppial.9.2.dylib"


# Relative path to `libnppig`
const libnppig_splitpath = ["lib", "libnppig.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppig = "@rpath/libnppig.9.2.dylib"
...
const libcupti_splitpath = ["lib", "libcupti.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcupti = "@rpath/libcupti.9.2.dylib"
...
const libcublas_splitpath = ["lib", "libcublas.9.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcublas = "@rpath/libcublas.9.2.dylib"
