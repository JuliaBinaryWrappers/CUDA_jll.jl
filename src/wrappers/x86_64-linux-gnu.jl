...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "libnppial.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "libnppicc.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "libcudart.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcusolver = "libcusolver.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "libnppist.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppig = "libnppig.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcusparse = "libcusparse.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "libnvblas.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "libnppidei.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "libnppisu.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppif = "libnppif.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcupti = "libcupti.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcublas = "libcublas.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppitc = "libnppitc.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "libcurand.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcufftw = "libcufftw.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "libnvgraph.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppicom = "libnppicom.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcufft = "libcufft.so.9.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppim = "libnppim.so.9.0"
