...
const libcufftw = "libcufftw.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcusolver = "libcusolver.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcusparse = "libcusparse.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "libcudart.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "libnppist.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppitc = "libnppitc.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppicom = "libnppicom.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppif = "libnppif.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "libnppicc.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "libcurand.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppim = "libnppim.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "libnppidei.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcupti = "libcupti.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcublas = "libcublas.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcufft = "libcufft.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "libnppial.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppig = "libnppig.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libcurand = "libnvgraph.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "libnvblas.so.10.0"
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "libnppisu.so.10.0"
