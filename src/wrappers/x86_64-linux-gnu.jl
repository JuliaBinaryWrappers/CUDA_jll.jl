...
const libcublas = "libcublas.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libcublasLt = "libcublasLt.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "libcudart.so.11.0"
...
const libcusolverMg_splitpath = ["lib", "libcusolverMg.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusparse = "libcusparse.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "libnppial.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "libnppicc.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "libnppidei.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "libnppist.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "libnppisu.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnppitc = "libnppitc.so.11"
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "libnvblas.so.11"
