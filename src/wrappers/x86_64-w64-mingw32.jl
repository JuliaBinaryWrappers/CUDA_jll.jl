...
const libcublasLt_splitpath = ["bin", "cublasLt64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcublasLt = "cublasLt64_10.dll"
...
const libcudart_splitpath = ["bin", "cudart64_102.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcudart = "cudart64_102.dll"
...
const libcupti_splitpath = ["bin", "cupti64_102.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcupti = "cupti64_102.dll"
...
const libcusolverMg_splitpath = ["bin", "cusolverMg64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcusolverMg = "cusolverMg64_10.dll"
...
const libnppc_splitpath = ["bin", "nppc64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppc = "nppc64_10.dll"


# Relative path to `libnppial`
const libnppial_splitpath = ["bin", "nppial64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppial = "nppial64_10.dll"


# Relative path to `libnppicc`
const libnppicc_splitpath = ["bin", "nppicc64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicc = "nppicc64_10.dll"


# Relative path to `libnppicom`
const libnppicom_splitpath = ["bin", "nppicom64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppicom = "nppicom64_10.dll"


# Relative path to `libnppidei`
const libnppidei_splitpath = ["bin", "nppidei64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppidei = "nppidei64_10.dll"


# Relative path to `libnppif`
const libnppif_splitpath = ["bin", "nppif64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppif = "nppif64_10.dll"


# Relative path to `libnppig`
const libnppig_splitpath = ["bin", "nppig64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppig = "nppig64_10.dll"


# Relative path to `libnppim`
const libnppim_splitpath = ["bin", "nppim64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppim = "nppim64_10.dll"


# Relative path to `libnppist`
const libnppist_splitpath = ["bin", "nppist64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppist = "nppist64_10.dll"


# Relative path to `libnppisu`
const libnppisu_splitpath = ["bin", "nppisu64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppisu = "nppisu64_10.dll"


# Relative path to `libnppitc`
const libnppitc_splitpath = ["bin", "nppitc64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnppitc = "nppitc64_10.dll"


# Relative path to `libnpps`
const libnpps_splitpath = ["bin", "npps64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnpps = "npps64_10.dll"


# Relative path to `libnvblas`
const libnvblas_splitpath = ["bin", "nvblas64_10.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnvblas = "nvblas64_10.dll"


# Relative path to `libnvgraph`
const libnvgraph_splitpath = ["bin", "nvgraph64_10.dll"]

# This will be filled out by __init__() for all products, as it must be done at runtime
libnvgraph_path = ""

# libnvgraph-specific global declaration
# This will be filled out by __init__()
libnvgraph_handle = C_NULL

# This must be `const` so that we can use it with `ccall()`
const libnvgraph = "nvgraph64_10.dll"
...
function __init__()
    ...
    global libnvgraph_path = normpath(joinpath(artifact_dir, libnvgraph_splitpath...))

    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libnvgraph_handle = dlopen(libnvgraph_path)
    push!(LIBPATH_list, dirname(libnvgraph_path))
