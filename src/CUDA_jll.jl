...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CUDA_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CUDA_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CUDA_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CUDA", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CUDA_jll", "src", "CUDA_jll.jl"))
        @info("CUDA_ll dev'ed out to /depot/dev/CUDA_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("e9e359dc-d701-5aa8-82ae-09bbf812ea83"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CUDA", artifacts_toml) for e in artifacts["CUDA"]]
...
if best_platform === nothing
    @debug("Unable to load CUDA; unsupported platform $(triplet(platform_key_abi()))")
else
